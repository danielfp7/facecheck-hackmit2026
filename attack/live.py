"""Live face swap in a plain full-screen window: the test attacker, without Deep-Live-Cam's UI.
  ...
  uv run --project server attack/live.py --source teammate.jpg
...
def main():
    ...
    ap.add_argument("--source", default=str(DEFAULT_SOURCE), help="photo of the face to swap in")
    ap.add_argument("--camera", type=int, default=0)
    ap.add_argument("--windowed", action="store_true", help="don't go full screen")
    args = ap.parse_args()
    if not Path(args.source).exists():
        sys.exit(f"no such photo: {args.source}")
